...
loader_params = {
    "use_fast_loader":False, # It is a queue loader to prefetch batch and storage.
    "max_prefetch":10,
    "batch_size":512,
    "shuffle":True, 
    "num_workers":4,
...
model_params = {
    ...
    "pooling":"serialized-attention", # statistics, lde, attentive, multi-head, multi-resolution, SelfAttention_multihead
    "pooling_params":{"num_nodes":256,
                      "num_head":1,
                      "share":True,
                      "affine_layers":6,
                      "hidden_size":64,
...
optimizer_params = {
    ...
    "beta2":0.999,
    "beta3":0.999,
    "weight_decay":5e-1,  # Should be large for decouped weight decay (adamW) and small for L2 regularization (sgd, adam).
    "lookahead.k":5,
...
epochs = 9 # Total epochs to train. It is important. #9
...
##--------------------------------------------------##
## Main params
traindata="data/mfcc_23_pitch/voxceleb2_train_aug"
egs_dir="exp/egs/mfcc_23_pitch_voxceleb2_train_aug" + "_" + sample_type

model_blueprint="subtools/pytorch/model/serialized-tdnn-xvector.py"
model_dir="exp/serialized_tdnn"
##--------------------------------------------------##
##
######################################################### START #########################################################
##
...
if stage <= 2 and endstage >= 0 and utils.is_main_training():
    ...
    kaldi_common.execute_command("subtools/pytorch/pipeline/preprocess_to_egs.sh "
...
if stage <= 4 <= endstage and utils.is_main_training():
    ...
    to_extracted_data = ["voxceleb2_train_aug", "sitw_dev_test", "cnceleb_train_aug", "cnceleb2_train_aug","cnceleb_eval_enroll", "cnceleb_eval_test", "sitw_dev_enroll", "sitw_eval_test", "sitw_eval_enroll", "voxceleb1_E", "voxceleb1_H"] 
        ...
        for position in to_extracted_positions:
            ...
            for epoch in to_extracted_epochs:
                ...
                for data in to_extracted_data:
                    ...
                    # Use a well-optimized shell script (with multi-p  rocesses) to extract xvectors.
