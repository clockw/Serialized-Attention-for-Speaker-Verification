...
class StatisticsPooling(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.stddev :
            ...
            # print(torch.cat((mean, std), dim=1).shape)
...
class AttentionAlphaComponent(torch.nn.Module):
    """Compute the alpha with attention module.
            alpha = softmax(v'路f(w路x + b) + k) or softmax(v'路x + k)
...
class SelfAttention(torch.nn.Module):
    ...
    def __init__(self, input_dim, affine_layers=2, hidden_size=64, context=[0], stddev=True, stddev_attention=True, eps=1.0e-10):
        ...
        if self.stddev :
            ...
            self.output_dim = input_dim
        self.dense_bn_relu1 = dense_bn_relu(512, 500)
        self.dense_bn_relu2 = dense_bn_relu(500, 1500)
        #self.dense_bn_relu3 = dense_bn_relu(1500, 1500)
        self.key_features = None
        self.value_features = None
        # self.queue = torch.randn(1, 1500)
        # self.queue = torch.nn.init.normal(self.queue, std=0.01)
        # self.queue = torch.nn.Parameter(self.queue)

        self.Wq = torch.nn.Parameter(torch.randn(3000, 1500))
        self.Wq = torch.nn.init.normal_(self.Wq, std=0.01)
    ...
    def forward(self, tdnn5, tdnn4):
        """
        @inputs: a 3-dimensional tensor (a batch), including [samples-index, frames-dim-index, frames-index]
        """
        # statistic pooling for var and mean
        counts = tdnn5.shape[2]
        mean = tdnn5.sum(dim=2, keepdim=True) / counts
        var = torch.sum((tdnn5 - mean)**2, dim=2, keepdim=True) / counts
        std = torch.sqrt(var.clamp(min=self.eps))
        mean_std = torch.cat((mean, std), dim=1).transpose(-1, -2)

        queue = torch.einsum('bht, td->hd', mean_std, self.Wq)
        self.key_features = self.dense_bn_relu1(tdnn4)
        self.key_features = self.dense_bn_relu2(self.key_features)
        self.value_features = tdnn5
        #self.value_features = self.dense_bn_relu3(tdnn5)
        self.key_features = self.key_features.unsqueeze(1)
        self.value_features = self.value_features.unsqueeze(1)
        dim = self.value_features.shape[-2]

        query_time_key = torch.einsum('bhdl, hd->bhl', self.key_features, queue)
        query_time_key = query_time_key * torch.rsqrt(torch.tensor(dim/1.0))
        weights = self.softmax(query_time_key)
        att_mean = torch.einsum('bhdl, bhl->bhd', self.value_features, weights)
        att_stddev = torch.einsum('bhdl, bhl->bhd', (self.value_features - att_mean.unsqueeze(3))**2, weights) 

        # forget!
        att_stddev = torch.sqrt(att_stddev.clamp(min=self.eps))

        return torch.cat((att_mean.transpose(-1, -2), att_stddev.transpose(-1, -2)), dim=1)
   

    def get_output_dim(self):
        return self.output_dim

class dense_bn_relu(torch.nn.Module):
    def __init__(self, num_features, num_nodes):
        super(dense_bn_relu, self).__init__()
        self.relu = torch.nn.ReLU()
       # self.dense = TdnnAffine(num_features, num_nodes, context=[0])
        self.dense = torch.nn.Conv1d(num_features, num_nodes, kernel_size=1)	
        self.bn = torch.nn.BatchNorm1d(num_nodes, momentum=0.99)
    def forward(self, x):
        x = self.dense(x)
        x = self.bn(x)
        x = self.relu(x)
